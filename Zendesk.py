...
def delete_selected(event, listbox, data_list):
    ...
    if sel:
        ...
        # Clean scheduler + tracking lists if applicable
        if i < len(data_list):
            job = data_list[i]
            ...
            if isinstance(job.get("time"), datetime):
                if job["time"] in scheduled_times:
                    scheduled_times.remove(job["time"])
                if job["time"] in scheduled_queue_times:
                    scheduled_queue_times.remove(job["time"])
...
message_box.bind("<Control-Shift-BackSpace>", lambda e: add_bullet(e, message_box))




def add_bullet(event=None, text_widget=None):
    if text_widget is None:
        return "break"

    try:
        # Get full selected lines
        ...
        # Decide: add or remove bullets
        all_bulleted = all(line.strip().startswith("•") for line in lines if line.strip())
        ...
        for line in lines:
            if not line.strip():
                ...
                new_lines.append(line[:bullet_index] + line[bullet_index + 1:].lstrip())
            else:
                # Add bullet
        ...
        # If no selection, fallback to current line only
        index = text_widget.index("insert linestart")
        current_line = text_widget.get(index, f"{index} lineend")

        if current_line.strip().startswith("•"):
            new_line = current_line.replace("•", "", 1).lstrip()
            text_widget.delete(index, f"{index} lineend")
            text_widget.insert(index, new_line)
        else:
            text_widget.delete(index, f"{index} lineend")
            text_widget.insert(index, f"• {current_line.strip()}")
...
def toggle_bold(event=None, text_widget=None):
    if text_widget is None:
        return "break"
    ...
    try:
        ...
        # Strip only newlines from edges but keep internal
        selected_clean = selected.strip("\n")

        if selected_clean.startswith("**") and selected_clean.endswith("**"):
            # Remove bold
            text_widget.delete(start, end)
            text_widget.insert(start, selected_clean[2:-2])
        else:
            # Apply bold in-place, preserve spacing and line
            text_widget.delete(start, end)
            text_widget.insert(start, f"**{selected_clean}**")
    except tk.TclError:
        pass  # No selection
...
def toggle_italic(event=None, text_widget=None):
    if text_widget is None:
        return "break"
    ...
    try:
        ...
        # ✅ If text is already italic (_text_), remove _..._
        if selected.startswith("_") and selected.endswith("_") and not selected.startswith("__"):
            text_widget.delete(start, end)
            text_widget.insert(start, selected[1:-1])
        else:
            # ✅ Wrap with single underscores
            text_widget.delete(start, end)
            text_widget.insert(start, f"_{selected}_")
...
message_box.bind("<Control-b>", lambda e: toggle_bold(e, message_box))
message_box.bind("<Control-i>", lambda e: toggle_italic(e, message_box))
    ...
    job_queue.append(
        {
            ...
            "raw_message": message,  # ✅ preserve formatting
...
def edit_job_popup(index, job_list, listbox, is_manual=False):
    ...
    message_box = CTkTextbox(popup, width=600, height=230)
    message_box.bind("<Control-Shift-BackSpace>", lambda e: add_bullet(e, message_box))
    message_box.bind("<Control-b>", lambda e: toggle_bold(e, message_box))
    message_box.bind("<Control-i>", lambda e: toggle_italic(e, message_box))
    ...
    # ✅ Use raw_message if available, else fallback to cleaned HTML
    raw_msg = job.get("raw_message")
    if raw_msg:
        message_box.insert("1.0", raw_msg)
    else:
    ...
    # Time picker (only for scheduled/manual jobs)
    if "time" in job:
    ...
    def save_changes():
        ...
        # ✅ Save raw_message and update formatted HTML version
        raw_message = message_box.get("1.0", tk.END).strip()
        job["raw_message"] = raw_message
        ...
        try:
            if "time" in job:
                ...
                # ✅ Replace updated job in list
                target_list = manual_jobs if is_manual else scheduled_jobs
...
scheduled_listbox.bind(
    "<Delete>", lambda e: delete_selected(e, scheduled_listbox, scheduled_jobs)
)
...
def add_manual_job():
    ...
    job = {
        ...
        "ticket": ticket,
        "message": html_message,
        "raw_message": message,  # ✅ preserve original
        "last_comment": last_comment,
        "check_last": check_last,
        "solve_ticket": solve_ticket,
        "public_reply": public_reply,
